...
use crate::{
    ...
    db::Db,
    error::AppError,
    models::{CacheInvalidateResponse, ReminderPreferences, SetPreferencesRequest},
};
...
/// `POST /api/cache/invalidate/{vault_id}`
///
/// Manually evict all cached entries for the given vault.  This is useful
/// after an on-chain state change that the backend has not yet observed via
/// a regular check-in or event.
pub async fn invalidate_cache(
    State(cache): State<Arc<VaultCache>>,
